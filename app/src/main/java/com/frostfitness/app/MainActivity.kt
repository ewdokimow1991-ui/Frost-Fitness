...
Screen.FOOD->FoodPage(p,foods,{foods=foods+it}){s=Screen.HOME};Screen.PROGRESS->ProgressPage(p,completed){s=Screen.HOME};Screen.PROFILE->ProfilePage(p){s=Screen.HOME}}}}}
...
@Composable fun Input(label:String,value:String,decimal:Boolean=false,set:(String)->Unit){OutlinedTextField(value,{raw->val normalized=raw.replace(',','.');val filtered=if(decimal)normalized.filterIndexed{i,c->c.isDigit()||(c=='.'&&i>0&&normalized.take(i).none{it=='.'})} else normalized.filter{it.isDigit()};set(filtered)},label={Text(label)},singleLine=true,keyboardOptions=KeyboardOptions(keyboardType=if(decimal)KeyboardType.Decimal else KeyboardType.Number),modifier=Modifier.fillMaxWidth())}
...
@Composable fun Home(p:Profile,completed:Int,start:(Workout)->Unit,food:()->Unit,progress:()->Unit,profile:()->Unit)=Page("Сегодня","${p.goal} • ${p.days}× в неделю"){Info("Фростик покажет технику каждого движения. Нажми на упражнение во время тренировки.");plan(p).forEach{w->Column(Modifier.fillMaxWidth().background(Card,RoundedCornerShape(18.dp)).padding(16.dp)){Text(w.name,fontWeight=FontWeight.Bold);Text("${w.exercises.size} упражнений",color=Muted);Button({start(w)},colors=ButtonDefaults.buttonColors(containerColor=Lime,contentColor=Color.Black)){Text("Начать")}}};Text("Завершено тренировок: $completed",color=Muted);Row(horizontalArrangement=Arrangement.spacedBy(6.dp)){OutlinedButton(food,Modifier.weight(1f)){Text("Питание",fontSize=11.sp)};OutlinedButton(progress,Modifier.weight(1f)){Text("Прогресс",fontSize=11.sp)};OutlinedButton(profile,Modifier.weight(1f)){Text("Профиль",fontSize=11.sp)}}}
@Composable fun WorkoutPage(w:Workout,finish:()->Unit,show:(Exercise)->Unit,back:()->Unit)=Page(w.name,"Нажми на упражнение — Фростик покажет движение."){var done by remember(w.name){mutableStateOf(setOf<Int>())};w.exercises.forEachIndexed{i,e->Row(Modifier.fillMaxWidth().background(Card,RoundedCornerShape(16.dp)).clickable{show(e)}.padding(14.dp),horizontalArrangement=Arrangement.SpaceBetween){Column(Modifier.weight(1f)){Text(e.name,fontWeight=FontWeight.Bold);Text(e.dose,color=Lime);Text("Фростик: показать технику ›",color=Muted,fontSize=12.sp)};Checkbox(i in done,{done=if(it)done+i else done-i})}};Go("Завершить",done.size==w.exercises.size,finish);OutlinedButton(back,Modifier.fillMaxWidth()){Text("Назад")}}
@Composable fun ExerciseCoach(e:Exercise,back:()->Unit)=Page(e.name,"${e.dose} • Фростик показывает технику"){FrostikMotion(e.name);Info(technique(e.name));Text("Повтори движение только в комфортной амплитуде. При боли остановись.",color=Muted);OutlinedButton(back,Modifier.fillMaxWidth()){Text("Вернуться к тренировке")}}
fun technique(n:String)=when(n){"Присед к стулу","Жим ногами"->"Стопы устойчиво. Колени идут по направлению носков. Опускайся подконтрольно, не округляя спину.";"Отжимания от опоры","Жим от груди"->"Корпус стабилен. Локти не разводи резко в стороны. Выжимай без рывка.";"Тяга верхнего блока"->"Грудь слегка вверх. Тяни локти вниз, не раскачивай корпус.";"Ягодичный мост"->"Подними таз за счёт ягодиц, сохраняя нейтральную поясницу. Вверху короткая пауза.";"Планка"->"Держи тело одной линией. Не проваливай поясницу.";else->"Держи корпус стабильно, выполняй движение плавно и в комфортной амплитуде."}

@Composable fun FrostikMotion(exercise:String){val tr=rememberInfiniteTransition(label="frostik");val phase by tr.animateFloat(0f,1f,infiniteRepeatable(tween(1500,easing=FastOutSlowInEasing),RepeatMode.Reverse),label="phase");Column(Modifier.fillMaxWidth().background(Card,RoundedCornerShape(22.dp)).padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){Text("ФРОСТИК • ДЕМОНСТРАЦИЯ",color=Lime,fontWeight=FontWeight.Bold);Canvas(Modifier.fillMaxWidth().height(250.dp)){val w=size.width;val h=size.height;val squat=if(exercise.contains("Присед")||exercise.contains("ногами"))phase else 0f;val push=if(exercise.contains("Отжим")||exercise.contains("груди"))phase else 0f;val bridge=if(exercise.contains("мост"))phase else 0f;val bob=sin(phase*3.14159f)*5f;val cx=w*.5f;val body=Color(0xFFE9EEF0);val skin=Color(0xFFFFC9A8);val hair=Color(0xFF202522);val hip=h*.60f+squat*42f-bridge*28f;val shoulder=h*.35f+squat*32f+push*15f-bridge*10f;drawCircle(skin,28f,Offset(cx,shoulder-48f+bob));drawArc(hair,190f,160f,false,Offset(cx-30f,shoulder-82f+bob),Size(60f,45f),style=Stroke(10f,cap=StrokeCap.Round));drawLine(body,Offset(cx,shoulder-18f),Offset(cx,hip),22f,StrokeCap.Round);val arms=55f+push*25f;drawLine(skin,Offset(cx,shoulder),Offset(cx-arms,shoulder+55f-push*25f),14f,StrokeCap.Round);drawLine(skin,Offset(cx,shoulder),Offset(cx+arms,shoulder+55f-push*25f),14f,StrokeCap.Round);val knee=h*.76f+squat*18f;drawLine(body,Offset(cx-8f,hip),Offset(cx-45f,knee),18f,StrokeCap.Round);drawLine(body,Offset(cx+8f,hip),Offset(cx+45f,knee),18f,StrokeCap.Round);drawLine(body,Offset(cx-45f,knee),Offset(cx-68f,h*.91f),16f,StrokeCap.Round);drawLine(body,Offset(cx+45f,knee),Offset(cx+68f,h*.91f),16f,StrokeCap.Round);drawCircle(Lime,5f,Offset(cx-10f,shoulder-52f+bob));drawCircle(Lime,5f,Offset(cx+10f,shoulder-52f+bob))};Text("Движение повторяется автоматически",color=Muted,fontSize=12.sp)}}
